...
import {
  ...
  ArrowLeftRight, RotateCcw, History, type LucideIcon,
} from "lucide-react";
...
const emptySlot = (): Slot => ({
  ...
  grade: null, doseRange: null, synergies: [], goalIngredients: [], error: null,
});
...
export default function ResearchPage() {
  ...
  // Default active section when sections load
  useEffect(() => {
  ...
  function restoreEntry(entry: HistoryEntry) {
    ...
    setMain({ query: entry.query, content: entry.content, streaming: false, sections,
      grade: extractGrade(entry.content), doseRange: extractDoseRange(entry.content),
      synergies: extractSynergies(entry.content), goalIngredients: extractGoalIngredients(entry.content), error: null });
  ...
  // Section tabs = union of A + B sections
            ...
            <div className="flex rounded-xl bg-gray-100 p-1 text-[13px]">
              {(["ingredient", "goal", "compare"] as const).map(m => (
                <button
                  ...
                  onClick={() => setMode(m)}
                  className={cn(
                    "flex items-center gap-1.5 rounded-lg px-4 py-2 font-medium capitalize transition",
          ...
          {mode !== "compare" ? (
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                disabled={!query.trim() || !compareQuery.trim()}
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {EXAMPLES_COMPARE.map(([a, b]) => (
                    <button
                      key={`${a}-${b}`}
                      type="button"
                      onClick={() => { setQuery(a); setCompareQuery(b); }}
  ...
  return (
    <div className="space-y-0">
              ...
              <div className="absolute right-0 z-20 mt-1.5 w-64 rounded-xl border border-black/[0.08] bg-white py-1.5 shadow-[0_8px_30px_rgba(0,0,0,0.1)]">
                ...
                {history.length > 0 && (
                  <button type="button"
                    ...
                    className="w-full border-t border-black/[0.05] px-3 py-2 text-left text-[11px] text-gray-400 hover:text-gray-600 transition-colors"
                  >
                    Clear history
                  </button>
                )}
      ...
      {/* ── STREAMING VIEW (before sections parse) ───────────────────────── */}
      {isStreaming && (
                ...
                <div key={label} className="rounded-xl border border-black/[0.06] bg-white p-6 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
                  {isCompareMode && (
                    <div className={cn("mb-4 flex items-center gap-2", accent === "brand" ? "text-brand" : "text-purple-600")}>
                      <span className={cn("rounded-md px-2 py-0.5 text-[10px] font-bold", accent === "brand" ? "bg-brand/10" : "bg-purple-100")}>
                        {label}
                      </span>
        ...
        <>
          ...
          <div className="mb-4 flex gap-0.5 overflow-x-auto rounded-xl border border-black/[0.06] bg-white p-1 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
            {allSectionHeadings.map(heading => {
              const { icon: Icon, iconCls } = getSectionCfg(heading);
          ...
          {/* Section content */}
          {isCompareMode ? (
            // Compare: same section, two columns
            <div className="grid grid-cols-2 gap-4">
              {[{ slot: main, label: "A", accent: "brand", labelCls: "bg-brand/10 text-brand" },
                { slot: compare, label: "B", accent: "purple", labelCls: "bg-purple-100 text-purple-600" }].map(({ slot, label, labelCls }) => {
                const section = slot.sections.find(s => s.heading === activeSection);
                const { icon: Icon, iconCls } = getSectionCfg(activeSection);
                return (
                  <div key={label} className="rounded-xl border border-black/[0.06] bg-white p-6 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
